...
add_action('admin_init', 'ai_wpdyn_handle_loadtest_report_submit');
...
function ai_wpdyn_handle_loadtest_report_submit() {
    if (!isset($_POST['ai_wpdyn_loadtest_report_submit'])) {
        return;
    }
    if (!current_user_can('manage_options')) {
        return;
    ...
    check_admin_referer('ai_wpdyn_sandbox_loadtest_report', 'ai_wpdyn_loadtest_report_nonce');
    $settings = ai_wpdyn_get_settings();

    $siteId = ai_wpdyn_sanitize_site_id(wp_unslash($_POST['ai_wpdyn_loadtest_site_id'] ?? ''));
    if ($siteId === '') {
        add_settings_error('ai_wpdyn_messages', 'ai_wpdyn_loadtest_missing_site', 'Load-test sharing requires Site ID.', 'error');
        return;
    }

    $workerId = sanitize_text_field(trim((string)wp_unslash($_POST['ai_wpdyn_loadtest_worker_id'] ?? '')));
    if ($workerId === '') {
        $workerId = ai_wpdyn_default_sandbox_agent_id($settings);
    }
    $strategy = ai_wpdyn_normalize_strategy(wp_unslash($_POST['ai_wpdyn_loadtest_strategy'] ?? 'edge-balanced'), 'edge-balanced');

    $samplesRaw = trim((string)wp_unslash($_POST['ai_wpdyn_loadtest_samples_json'] ?? ''));
    if ($samplesRaw === '') {
        add_settings_error('ai_wpdyn_messages', 'ai_wpdyn_loadtest_missing_samples', 'Paste a JSON array of page tests first.', 'error');
        return;
    }

    $decoded = json_decode($samplesRaw, true);
    if (!is_array($decoded)) {
        add_settings_error('ai_wpdyn_messages', 'ai_wpdyn_loadtest_invalid_json', 'Load-test JSON must be a valid array.', 'error');
        return;
    }

    $response = ai_wpdyn_report_shared_loadtests($settings, [
        'site_id' => $siteId,
        'worker_id' => $workerId,
        'strategy' => $strategy,
        'page_tests' => $decoded,
    ]);
    list($status, $body, $errorText) = ai_wpdyn_decode_worker_json_response($response);

    if ($status >= 200 && $status < 300 && !empty($body['ok'])) {
        $inserted = (int)($body['inserted'] ?? 0);
        $failed = (int)($body['hard_gate_failed'] ?? 0);
        add_settings_error(
            'ai_wpdyn_messages',
            'ai_wpdyn_loadtest_report_ok',
            'Shared load tests saved. Inserted: ' . $inserted . ', hard-gate failed: ' . $failed . '.',
            'updated'
        );
        return;
    }
    ...
    if ($errorText === '') {
        $errorText = 'Unable to publish shared load tests.';
    }
    add_settings_error('ai_wpdyn_messages', 'ai_wpdyn_loadtest_report_failed', 'Load-test sharing failed: ' . $errorText, 'error');
}
...
function ai_wpdyn_render_settings_page() {
    ...
    $loadtestSiteFilter = ai_wpdyn_sanitize_site_id(wp_unslash($_GET['ai_wpdyn_loadtest_site'] ?? $settings['site_id']));
    $loadtestPathFilter = sanitize_text_field(trim((string)wp_unslash($_GET['ai_wpdyn_loadtest_path'] ?? '')));
    $loadtestStrategyFilterRaw = strtolower(trim((string)wp_unslash($_GET['ai_wpdyn_loadtest_strategy'] ?? 'all')));
    $loadtestAllowedStrategies = array_merge(['all'], ai_wpdyn_available_strategies());
    $loadtestStrategyFilter = in_array($loadtestStrategyFilterRaw, $loadtestAllowedStrategies, true) ? $loadtestStrategyFilterRaw : 'all';
    $loadtestOnlyPassing = !isset($_GET['ai_wpdyn_loadtest_only_passing']) || (int)wp_unslash($_GET['ai_wpdyn_loadtest_only_passing']) === 1;
    ...
    if ($sandboxConfigReady) {
        ...
        $loadtestResponse = ai_wpdyn_list_shared_loadtests($settings, [
            'site_id' => ($loadtestSiteFilter !== '') ? $loadtestSiteFilter : null,
            'page_path' => ($loadtestPathFilter !== '') ? $loadtestPathFilter : null,
            'strategy' => ($loadtestStrategyFilter !== 'all') ? $loadtestStrategyFilter : null,
            'only_passing' => $loadtestOnlyPassing,
            'limit' => 200,
        ]);
        list($loadtestStatus, $loadtestBody, $loadtestErrorText) = ai_wpdyn_decode_worker_json_response($loadtestResponse);
        if (
            $loadtestStatus >= 200 &&
            $loadtestStatus < 300 &&
            !empty($loadtestBody['ok']) &&
            is_array($loadtestBody['shared_page_profiles'] ?? null)
        ) {
            $sharedPageProfiles = $loadtestBody['shared_page_profiles'];
            if (is_array($loadtestBody['strategy_leaderboard'] ?? null)) {
                $sharedStrategyLeaderboard = $loadtestBody['strategy_leaderboard'];
            }
        } else {
            $sharedLoadtestError = ($loadtestErrorText !== '') ? $loadtestErrorText : 'Unable to fetch shared page-load tests.';
        }
    }
    ...
    <div class="wrap">
      ...
      <hr />
      <h2>Shared Page Load Tests</h2>
      <p>Publish per-page speed tests from each worker so strategy selection can use shared real-world latency across the fleet.</p>

      <h3>Publish Page Tests</h3>
      <form method="post">
        <?php wp_nonce_field('ai_wpdyn_sandbox_loadtest_report', 'ai_wpdyn_loadtest_report_nonce'); ?>
        <table class="form-table" role="presentation">
          <tr>
            <th scope="row"><label for="ai_wpdyn_loadtest_site_id">Site ID</label></th>
            <td><input type="text" class="regular-text" id="ai_wpdyn_loadtest_site_id" name="ai_wpdyn_loadtest_site_id" value="<?php echo esc_attr((string)$settings['site_id']); ?>" required /></td>
          </tr>
          <tr>
            <th scope="row"><label for="ai_wpdyn_loadtest_worker_id">Worker ID</label></th>
            <td><input type="text" class="regular-text" id="ai_wpdyn_loadtest_worker_id" name="ai_wpdyn_loadtest_worker_id" value="<?php echo esc_attr($defaultAgentId); ?>" /></td>
          </tr>
          <tr>
            <th scope="row"><label for="ai_wpdyn_loadtest_strategy">Strategy</label></th>
            <td>
              <select id="ai_wpdyn_loadtest_strategy" name="ai_wpdyn_loadtest_strategy">
                <?php foreach (ai_wpdyn_available_strategies() as $strategy): ?>
                <option value="<?php echo esc_attr($strategy); ?>"><?php echo esc_html($strategy); ?></option>
                <?php endforeach; ?>
              </select>
            </td>
          </tr>
          <tr>
            <th scope="row"><label for="ai_wpdyn_loadtest_samples_json">Page Tests JSON</label></th>
            <td>
              <textarea id="ai_wpdyn_loadtest_samples_json" name="ai_wpdyn_loadtest_samples_json" class="large-text code" rows="8">[
  {
    ...
    "gates": {
      ...
      "cache_key_collision": false
              ...
              <p class="description">Use an array of per-page objects from k6/Lighthouse or worker probes. These samples are shared to D1 for cross-worker optimization.</p>
            </td>
          </tr>
        </table>
        <p class="submit"><button type="submit" class="button button-secondary" name="ai_wpdyn_loadtest_report_submit">Publish Shared Load Tests</button></p>
      </form>

      <h3>Shared Feed</h3>
      <form method="get" style="margin-bottom: 12px;">
        <input type="hidden" name="page" value="<?php echo esc_attr(AI_WPDYN_MENU_SLUG); ?>" />
        <label for="ai_wpdyn_loadtest_site"><strong>Site ID</strong></label>
        <input type="text" class="regular-text" id="ai_wpdyn_loadtest_site" name="ai_wpdyn_loadtest_site" value="<?php echo esc_attr($loadtestSiteFilter); ?>" />
        <label for="ai_wpdyn_loadtest_path"><strong>Page Path</strong></label>
        <input type="text" class="regular-text code" id="ai_wpdyn_loadtest_path" name="ai_wpdyn_loadtest_path" value="<?php echo esc_attr($loadtestPathFilter); ?>" placeholder="/pricing/" />
        <label for="ai_wpdyn_loadtest_strategy_filter"><strong>Strategy</strong></label>
        <select id="ai_wpdyn_loadtest_strategy_filter" name="ai_wpdyn_loadtest_strategy">
          <option value="all" <?php selected($loadtestStrategyFilter, 'all'); ?>>all</option>
          <?php foreach (ai_wpdyn_available_strategies() as $strategy): ?>
          <option value="<?php echo esc_attr($strategy); ?>" <?php selected($loadtestStrategyFilter, $strategy); ?>><?php echo esc_html($strategy); ?></option>
          <?php endforeach; ?>
        </select>
        <label><input type="checkbox" name="ai_wpdyn_loadtest_only_passing" value="1" <?php checked($loadtestOnlyPassing, true); ?> /> only passing hard gates</label>
        <button type="submit" class="button">Refresh Shared Feed</button>
      </form>

      <?php if ($sandboxConfigReady && $sharedLoadtestError !== ''): ?>
      <div class="notice notice-error inline"><p><?php echo esc_html('Shared load-test feed error: ' . $sharedLoadtestError); ?></p></div>
      <?php endif; ?>

      <?php if ($sandboxConfigReady && $sharedLoadtestError === ''): ?>
        <?php if (empty($sharedPageProfiles)): ?>
        <p><em>No shared page tests found for current filters.</em></p>
        <?php else: ?>
        <table class="widefat striped">
          ...
          <tbody>
            <?php foreach ($sharedPageProfiles as $profile): ?>
            <tr>
              <td><code><?php echo esc_html((string)($profile['page_path'] ?? '')); ?></code></td>
              <td><?php echo esc_html((string)($profile['strategy'] ?? '')); ?></td>
              <td><?php echo esc_html((string)($profile['sample_count'] ?? '0')); ?></td>
              <td><?php echo esc_html(number_format((float)($profile['avg_p95_latency_ms'] ?? 0), 2)); ?></td>
              <td><?php echo esc_html(number_format((float)($profile['avg_edge_hit_ratio'] ?? 0), 3)); ?></td>
              <td><?php echo esc_html(number_format((float)($profile['avg_score'] ?? 0), 4)); ?></td>
              <td><?php echo esc_html(number_format((float)($profile['pass_ratio'] ?? 0), 3)); ?></td>
              <td><?php echo esc_html((string)($profile['last_seen_at'] ?? '')); ?></td>
            </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
        ...
        <?php if (!empty($sharedStrategyLeaderboard)): ?>
        <h4 style="margin-top:16px;">Strategy Leaderboard (Shared)</h4>
        <table class="widefat striped">
          <thead>
            <tr>
              <th>Strategy</th>
          ...
          <tbody>
            <?php foreach ($sharedStrategyLeaderboard as $row): ?>
            <tr>
              <td><?php echo esc_html((string)($row['strategy'] ?? '')); ?></td>
              <td><?php echo esc_html((string)($row['sample_count'] ?? '0')); ?></td>
              <td><?php echo esc_html(number_format((float)($row['avg_p95_latency_ms'] ?? 0), 2)); ?></td>
              <td><?php echo esc_html(number_format((float)($row['avg_score'] ?? 0), 4)); ?></td>
              <td><?php echo esc_html(number_format((float)($row['pass_ratio'] ?? 0), 3)); ?></td>
            </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
...
function ai_wpdyn_report_shared_loadtests($settings, $payload) {
    return ai_wpdyn_signed_mutation_post($settings, '/plugin/wp/sandbox/loadtests/report', $payload, 20);
}

function ai_wpdyn_list_shared_loadtests($settings, $payload) {
    return ai_wpdyn_signed_mutation_post($settings, '/plugin/wp/sandbox/loadtests/shared', $payload, 12);
}
